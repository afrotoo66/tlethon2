#all write Codes By Team Aljoker @T_Y_E_X
#By Hussein @lMl10l
import asyncio
import random
import json
    ...
    async with bot.conversation("@SpamBot") as conv:
        try:
            response = conv.wait_event(
                events.NewMessage(incoming=True, from_users=5904216848)
            )
...
async def aljoker313(joker313):
  ...
  await joker313.client.send_file(joker313.chat_id,url,caption="᯽︙ BY : @T_Y_E_X 🎀",parse_mode="html")
...
async def jepvois(vois):
  ...
  await vois.client.send_file(vois.chat_id,url,caption="᯽︙ BY : @T_Y_E_X 🎀",parse_mode="html")
  await vois.delete()
@l313l.on(admin_cmd(outgoing=True, pattern="قران$"))
async def jepvois(vois):
  rl = random.randint(2,101)
  url = f"https://t.me/QuraanJep/{rl}"
  await vois.client.send_file(vois.chat_id,url,caption="᯽︙ BY : @T_Y_E_X 🤲🏻☪️",parse_mode="html")
  await vois.delete()
@l313l.on(admin_cmd(outgoing=True, pattern="ثيم$"))
async def jepThe(theme):
  rl = random.randint(2,510)
  url = f"https://t.me/GSSSD/{rl}"
  await theme.client.send_file(theme.chat_id,url,caption="᯽︙ THEME BY : @T_Y_E_X 🎊",parse_mode="html")
  await theme.delete()
